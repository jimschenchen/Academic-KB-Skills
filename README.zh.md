...
存放自主开发和维护的 Claude Skills 源码。每个 skill 是一个独立文件夹，包含 SKILL.md（核心指令）、references/（模板和参考资料）以及文档。
...
### 1. academic-kb — 学术版 Karpathy LLM Wiki

基于 Obsidian 的学术知识库全流程管理 skill：ingest → read → compile → query → lint。支持学术论文（PDF）和文章（博客、技术报告、白皮书、社交媒体）。

灵感来源：

* **Karpathy 的知识库工作流** — ingest → compile → query → lint 管线的核心理念
* **Paperwise (sjqsgg)** — 双栏 HTML 标注设计与 5 色高亮系统
